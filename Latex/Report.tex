...
Using the simpler scheme reduces the effort required to determine a useful delay for the gate.
The input conditioning stage reduces the unrealistic effects of the infinite drive strength of the pulse source.
Because there is no loading on the output the delays will be substantially less than the proper FO4 circuit.
Table \ref{tab:tran_dim} summarizes the measurements, as well as the physical dimensions of each gate.

\begin{table}[H]
	\centering
	\caption{Worst Case Transistor Delay, Rise, Fall Times, and Gate Dimensions}
	\label{tab:tran_dim}
