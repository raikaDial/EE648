...
Delay is measured from 50\% on the input to 50\% on the output, the rise time is measured from 20\% on the output to 80\% on the output, and fall time is measured from 80\% on the output to 20\% on the output.
For the two, three, and four input NAND gates, the circuits used to test the delay, rise, and fall times consisted of the device under test, with an inverter on each input and the output driving four inputs to a circuit of the same type.
Figure \ref{} shows this setup for the ""-input NAND.
For the inverter circuit, the fan-out-of-four delay (fo4) was measured; the setup for this is shown in Figure \ref{}
Table \ref{tab:tran_dim} shows these measurements, as well as the physical dimensions of each gate.

\begin{table}[H]
	\centering
	\caption{Select Transistor Properties}
	\label{tab:tran_dim}
	\begin{tabular}{lccccccc}
		\toprule
		Gate & \begin{tabular}{@{}c@{}}Falling \\ Delay \end{tabular} & \begin{tabular}{@{}c@{}}Rising \\ Delay \end{tabular} & Rise & Fall & Width & Height & Area\\
		\midrule
		INV		& ps & ps & ps & ps &	128\(\lambda\) &	86\(\lambda\)	& 11,008\(\lambda\)	\\
		2NAND	&  & ps &      &      &	128\(\lambda\)	&	90\(\lambda\) 	& 11,520\(\lambda\)	\\
		3NAND	&&&&&	128\(\lambda\) &	94\(\lambda\) 	& 12,032\(\lambda\)	\\
		4NAND	&&&&&	128\(\lambda\)	&	98\(\lambda\) 	& 12,544\(\lambda\)	\\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\textwidth, keepaspectratio]{Graphics/1INV_Timing}
	\caption{Measuring Delay and Rise/Fall Times}
\end{figure}
